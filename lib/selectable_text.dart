...
class SelectableController {
  VoidCallback? clearSelectableState;
  bool selected = false;

  dispose() {
    clearSelectableState = null;
    selected = false;
  }
}
...
class _SelectableTextSelectionGestureDetectorBuilder extends TextSelectionGestureDetectorBuilder {
  ...
  void onSingleLongTapStart(LongPressStartDetails details) {
    print("on single Long tap up start");

    if (delegate.selectionEnabled) {
      renderEditable.selectWord(cause: SelectionChangedCause.longPress);
      Feedback.forLongPress(_state.context);
    }
    _state.widget.controller?.selected = true;
    _state.widget.onLongPressStart?.call(details);
  }
}

/// A run of selectable text with a single style.
///
/// The [CusSelectableText] widget displays a string of text with a single style.
...
/// {@end-tool}
///
/// Using the [CusSelectableText.rich] constructor, the [CusSelectableText] widget can
...
/// ## Interactivity
///
/// To make [CusSelectableText] react to touch events, use callback [onTap] to achieve
...
class CusSelectableText extends StatefulWidget {
  ...
  const CusSelectableText(
    ...
    this.magnifierConfiguration, required this.controller, this.onLongPressStart, this.onDoubleTapDown,
  ...
  final GestureDoubleTapCallback? onDoubleTapDown;
  ...
  @override
  State<CusSelectableText> createState() => _SelectableTextState();
...
class _SelectableTextState extends State<CusSelectableText> implements TextSelectionGestureDetectorBuilderDelegate {
  ...
  void initState() {
    ...
    widget.controller?.clearSelectableState = ()=>_clearTextSelectedStatus();
  }

  @override
  void didUpdateWidget(CusSelectableText oldWidget) {
  ...
  void _clearTextSelectedStatus(){
    _editableText?.hideToolbar();
    if (selectionEnabled) {
      switch (Theme.of(context).platform) {
        case TargetPlatform.iOS:
        case TargetPlatform.macOS:
          _editableText?.renderEditable.selectWordEdge(cause: SelectionChangedCause.tap);
        case TargetPlatform.android:
        case TargetPlatform.fuchsia:
        case TargetPlatform.linux:
        case TargetPlatform.windows:
          _editableText?.renderEditable.selectPosition(cause: SelectionChangedCause.tap);
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    final Widget child = RepaintBoundary(
      child: EditableText(
        ...
        onTapOutside: (event) {if (widget.controller?.selected ?? false) { _clearTextSelectedStatus();}},
